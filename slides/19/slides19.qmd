...
2. Copy and paste your census key into the empty file, followed by an empty line
...
::: {.column .nonincremental width="50%"}
- `geography = "county"`: What level of detail to get summary statistics. Other options include "state", "tract", "combined statistical area", etc.
...
Getting variables from the Census or ACS requires knowing the variable ID - and there are thousands of these IDs across the different Census files. You can search for these online using `data.census.gov/table/` or with `load_variables` from the {tidycensus} package
